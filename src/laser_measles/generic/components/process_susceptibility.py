...
class SusceptibilityProcess:
    ...
    def on_birth(self, model, _tick, istart, iend):
        ...
        if iend is not None:
            SusceptibilityProcess.nb_set_susceptibility_slice(istart, iend, model.population.susceptibility, np.uint8(1))
        else:
            SusceptibilityProcess.nb_set_susceptibility_randomaccess(istart, model.population.susceptibility, np.uint8(1))
