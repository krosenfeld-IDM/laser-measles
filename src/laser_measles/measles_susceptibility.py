...
class Susceptibility:
    ...
    def plot(self) -> None:
        img = plt.figure(figsize=(12, 9), dpi=128)
        img.suptitle("Susceptibility Distribution By Age")
...
def setup_susceptibility(model, verbose: bool = False) -> None:
    model.population.add_scalar_property("susceptibility", dtype=np.uint8, default=0)  # most agents are initially not susceptible
...
def initialize_susceptibility(count, dob, susceptibility) -> None:
    ...
    for i in nb.prange(count):
        susceptibility[i] = 0 if -dob[i] > five_years else 1
