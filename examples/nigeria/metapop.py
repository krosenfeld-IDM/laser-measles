...
def get_scenario(params, verbose: bool = False) -> pd.DataFrame:
    ...
    if verbose:
        print(f"Loading population and location data from '{params.shape_file}'…")
    gpdf = shapefiles.get_shapefile_dataframe(params.shape_file).to_pandas()
    if verbose:
        print(f"Loaded {len(gpdf):,} patches (total population {gpdf.population.sum():,}) from '{params.shape_file}'.")
